...
// Protege o /leads com password simples (autenticação HTTP Basic).
// Define LEADS_USER e LEADS_PASSWORD no .env / nas variáveis de ambiente do Render.
function requireLeadsAuth(req, res, next) {
  const expectedUser = process.env.LEADS_USER;
  const expectedPass = process.env.LEADS_PASSWORD;

  if (!expectedUser || !expectedPass) {
    res.status(500).send('LEADS_USER e LEADS_PASSWORD não estão configurados no servidor.');
    return;
  }
...
app.get('/leads', requireLeadsAuth, (req, res) => {
  try {
...
async function downloadImageAsBase64(url, fallbackMimeType) {
  ...
  const mimeType = res.headers.get('content-type') || fallbackMimeType || 'image/jpeg';
  const arrayBuffer = await res.arrayBuffer();
  const base64 = Buffer.from(arrayBuffer).toString('base64');
  return { mimeType, base64 };
}

async function handleIncomingMessage(client, chatId, userContent) {
    ...
    try {
      ...
      try {
        if (body?.typeWebhook === 'incomingMessageReceived') {
          ...
          if (chatId && client.notifyNumber && chatId === client.notifyNumber) {
            console.log(`[${client.id}] Mensagem do staff ignorada (não é conversa de cliente).`);
          } else if (chatId && (typeMessage === 'textMessage' || typeMessage === 'extendedTextMessage')) {
            const userText =
          ...
          } else if (chatId && typeMessage === 'imageMessage') {
            try {
              const downloadUrl = msgData.fileMessageData?.downloadUrl;
              const caption = msgData.fileMessageData?.caption || '';
              const mimeType = msgData.fileMessageData?.mimeType;
              if (downloadUrl) {
                const { mimeType: detectedMime, base64 } = await downloadImageAsBase64(downloadUrl, mimeType);
                const contentBlocks = [
                  { type: 'image', source: { type: 'base64', media_type: detectedMime, data: base64 } },
                  { type: 'text', text: caption || 'O cliente enviou esta imagem, sem legenda.' }
                ];
                await handleIncomingMessage(client, chatId, contentBlocks);
              }
            } catch (imgErr) {
              console.error(`[${client.id}] Erro a processar imagem:`, imgErr.message);
              await sendGreenApiMessage(client, chatId, 'Recebi a tua imagem mas tive um problema técnico a abri-la — podes tentar enviar outra vez?');
            }
          } else if (chatId && (typeMessage === 'audioMessage' || typeMessage === 'pttMessage')) {
            try {
              const downloadUrl = msgData.fileMessageData?.downloadUrl;
              const mimeType = msgData.fileMessageData?.mimeType;

              if (downloadUrl && process.env.OPENAI_API_KEY) {
                const transcript = await transcribeAudio(downloadUrl, mimeType);
                if (transcript && transcript.trim()) {
                  await handleIncomingMessage(client, chatId, transcript.trim());
                } else {
                  await sendGreenApiMessage(client, chatId, 'Recebi o teu áudio mas não consegui perceber o que foi dito — podes tentar outra vez ou escrever, por favor?');
                }
              } else {
                await sendGreenApiMessage(client, chatId, 'Recebi o teu áudio! Por agora só consigo perceber mensagens de texto ou fotos — podes escrever a tua pergunta, por favor? 🙏');
              }
            } catch (audioErr) {
              console.error(`[${client.id}] Erro a transcrever áudio:`, audioErr.message);
              await sendGreenApiMessage(client, chatId, 'Recebi o teu áudio mas tive um problema técnico a percebê-lo — podes escrever a tua pergunta, por favor?');
            }
          }
        }
      } catch (procErr) {
